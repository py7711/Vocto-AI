...
const authCopy = {
  zh: {
    ...
    securityEyebrow: "企业级安全工作区",
    securityPoints: ["短期上传签名，媒体文件直传对象存储。", "额度、任务队列、AI 洞察、翻译和导出统一进入仪表盘。", "认证架构预留 OAuth、邮箱验证、Stripe 和企业 SSO。"],
    verifyTitle: "验证您的邮箱",
  ...
  en: {
    signupTitle: "Create your Votxt account",
    signinTitle: "Sign in to Votxt",
    ...
    switchToSignup: "No account yet? Sign up free",
    securityEyebrow: "Enterprise-grade secure workspace",
    securityPoints: ["Short-lived upload signatures send media directly to object storage.", "Quota, queues, AI insights, translation, and exports live in one dashboard.", "The auth architecture is ready for OAuth, email verification, Stripe, and enterprise SSO."],
    ...
    signinHeading: "Sign in to Votxt",
  ...
  es: {
    ...
    securityEyebrow: "Workspace seguro de nivel empresarial",
    securityPoints: ["Subidas firmadas de corta duración hacia almacenamiento de objetos.", "Cuotas, cola, IA, traducción y exportaciones en un solo panel.", "Arquitectura lista para OAuth, verificación de email, Stripe y SSO empresarial."],
  ...
  fr: {
    ...
    securityEyebrow: "Espace sécurisé niveau entreprise",
    securityPoints: ["Signatures d'upload courtes vers stockage objet.", "Quota, file, IA, traduction et exports dans un seul tableau.", "Architecture prête pour OAuth, email, Stripe et SSO entreprise."],
  ...
  de: {
    ...
    securityEyebrow: "Sicherer Enterprise-Arbeitsbereich",
    securityPoints: ["Kurzlebige Upload-Signaturen für Objektspeicher.", "Kontingent, Warteschlange, KI, Übersetzung und Exporte in einem Dashboard.", "Bereit für OAuth, E-Mail-Verifizierung, Stripe und Enterprise-SSO."],
  ...
  ja: {
    ...
    securityEyebrow: "エンタープライズ級セキュアワークスペース",
    securityPoints: ["短期アップロード署名でオブジェクトストレージへ直接送信。", "クォータ、キュー、AI、翻訳、書き出しを1つの画面で管理。", "OAuth、メール検証、Stripe、企業SSOに対応できる認証基盤。"],
  ...
  ko: {
    ...
    securityEyebrow: "엔터프라이즈급 보안 워크스페이스",
    securityPoints: ["짧은 업로드 서명으로 미디어를 객체 저장소에 직접 전송합니다.", "할당량, 큐, AI, 번역, 내보내기를 하나의 대시보드에서 관리합니다.", "OAuth, 이메일 인증, Stripe, 기업 SSO를 위한 인증 구조입니다."],
    verifyTitle: "이메일 인증",
  ...
  pt: {
    ...
    securityEyebrow: "Workspace seguro empresarial",
    securityPoints: ["Assinaturas curtas enviam mídia direto ao armazenamento.", "Cotas, fila, IA, tradução e exportações em um painel.", "Arquitetura pronta para OAuth, verificação de email, Stripe e SSO empresarial."],
...
export function AuthPage({mode}: {mode: "signin" | "signup"}) {
  ...
  return (
    <main className="min-h-screen px-4 py-10 md:px-8">
      <section className="mx-auto grid min-h-[calc(100vh-64px)] max-w-3xl content-center">
        <div className="animate-fade-up rounded-3xl border border-ink/10 bg-white/90 px-6 py-10 shadow-lifted backdrop-blur md:px-16 md:py-14">
          <div className="mb-12 flex justify-center">
            <Image src="/votxt-logo.svg" alt="Votxt" width={190} height={52} priority />
          </div>
          {isSignup ? (
            <SignupCard
              locale={locale}
          ...
          <div className="mt-10 hidden rounded-2xl bg-paper/65 p-4 text-sm leading-6 text-ink/60 ring-1 ring-ink/5 md:block">
            <p className="flex items-center gap-2 font-black text-ink/72">
              <ShieldCheck size={16} className="text-tide" />
              {text.securityEyebrow}
            </p>
            <div className="mt-2 grid gap-1">
              {text.securityPoints.map((item) => <p key={item}>{item}</p>)}
            </div>
          </div>
        </div>
      </section>
...
function GoogleMark() {
  return <span className="text-3xl font-black"><span className="text-blue-500">G</span></span>;
}

function Divider({label}: {label: string}) {
  return (
    <div className="my-7 flex items-center gap-4 text-sm font-bold text-ink/45">
...
function GoogleButton({locale, label}: {locale: string; label: string}) {
  return (
    <a href={`/api/auth/google/start?locale=${locale}`} className="focus-ring mt-8 flex w-full items-center justify-center gap-5 rounded-2xl border border-ink/15 bg-white px-4 py-4 text-xl font-black text-ink shadow-soft transition hover:border-ink/30 hover:shadow-card">
      <GoogleMark />
      {label}
    </a>
  );
...
function SignupCard({
  ...
  async function submitRegister() {
    ...
    try {
      const response = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({email, firstName, lastName, password, locale})
      });
  ...
  if (step === "verify") {
    return (
      <div className="text-center">
        <Mail className="mx-auto text-tide" size={58} />
        ...
        <a href={`/${locale}/dashboard`} className="focus-ring mt-8 flex w-full items-center justify-center gap-2 rounded-2xl bg-tide px-5 py-4 text-lg font-black text-white shadow-soft transition hover:brightness-110">
          {text.goDashboard}
        </a>
        {verificationUrl ? (
          <div className="mt-5 rounded-2xl border border-tide/20 bg-tide/5 p-4 text-left">
            <p className="text-sm font-bold leading-6 text-ink/65">{text.devVerifyHint}</p>
            <a href={verificationUrl} className="mt-3 inline-flex break-all text-sm font-black text-tide">{text.openVerifyLink}</a>
          </div>
        ) : null}
        <a href={`/${locale}/auth/signin`} className="focus-ring mt-0 flex w-full items-center justify-center gap-2 rounded-xl border border-ink/15 px-5 py-4 text-lg font-black text-tide">
          <ArrowLeft size={19} />
          {text.backToSignin}
        </a>
  ...
  return (
    <div>
      <h1 className="text-center text-4xl font-black text-ink">{text.signupHeading}</h1>
      <GoogleButton locale={locale} label={text.google} />
      <Divider label={text.signupDivider} />
      {step === "email" ? (
        <>
          <label className="block">
            <span className="mb-3 block text-lg font-black">{text.email}</span>
            <input
              type="email"
              value={email}
              onChange={(event) => setEmail(event.target.value)}
              className="focus-ring w-full rounded-xl border border-ink/15 px-5 py-4 text-xl"
              placeholder="gxx123@gmail.com"
            />
          </label>
          <button type="button" onClick={() => setStep("profile")} disabled={!email.includes("@")} className="focus-ring mt-6 w-full rounded-2xl bg-tide px-5 py-4 text-lg font-black text-white shadow-soft transition hover:brightness-110 disabled:opacity-45">
            {text.continue}
          </button>
        </>
      ) : (
        <>
          <div className="mb-6 rounded-2xl bg-ink/[0.03] p-4 ring-1 ring-ink/5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-bold text-ink/50">{text.emailLabel}</p>
                <p className="break-words text-lg font-black">{email}</p>
              </div>
              <button type="button" onClick={() => setStep("email")} className="font-black text-tide">{text.edit}</button>
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <label>
              <span className="mb-2 block text-base font-black">{text.firstName}</span>
              <input value={firstName} onChange={(event) => setFirstName(event.target.value)} className="focus-ring w-full rounded-xl border border-ink/15 px-4 py-4 text-lg" placeholder={text.firstNamePlaceholder} />
            </label>
            <label>
              <span className="mb-2 block text-base font-black">{text.lastName}</span>
              <input value={lastName} onChange={(event) => setLastName(event.target.value)} className="focus-ring w-full rounded-xl border border-ink/15 px-4 py-4 text-lg" placeholder={text.lastNamePlaceholder} />
            </label>
          </div>
          <label className="mt-5 block">
            <span className="mb-2 block text-base font-black">{text.password}</span>
            <div className="flex rounded-xl border border-ink/15">
          ...
          {error ? <p className="mt-4 animate-fade-in rounded-xl border border-coral/30 bg-coral/10 px-3 py-2 text-sm font-bold text-coral">{error}</p> : null}
          <button type="button" onClick={submitRegister} disabled={busy || password.length < 8} className="focus-ring mt-6 w-full rounded-2xl bg-tide px-5 py-4 text-lg font-black text-white shadow-soft transition hover:brightness-110 disabled:opacity-45">
      ...
      <p className="mt-8 text-center text-base text-ink/55">{text.already} <a className="font-black text-tide" href={`/${locale}/auth/signin`}>{text.signinLink}</a></p>
      <p className="mx-auto mt-6 max-w-sm text-center text-sm leading-6 text-ink/55">{text.termsPrefix} <a className="font-bold text-tide" href={`/${locale}/terms`}>{text.terms}</a> {text.and} <a className="font-bold text-tide" href={`/${locale}/privacy`}>{text.privacy}</a>.</p>
    </div>
  );
}
  ...
  return (
    <main className="min-h-screen px-4 py-10 md:px-8">
      <section className="mx-auto grid min-h-[calc(100vh-64px)] max-w-xl content-center">
        <div className="animate-fade-up rounded-3xl border border-ink/10 bg-white/90 p-8 text-center shadow-lifted backdrop-blur md:p-12">
          <div className="mb-12 flex justify-center">
            <Image src="/votxt-logo.svg" alt="Votxt" width={190} height={52} priority />
          </div>
          <Mail className="mx-auto text-tide" size={68} />
          ...
          <a href={`/${locale}/dashboard`} className="focus-ring mt-8 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-tide px-5 py-4 text-lg font-black text-white shadow-soft transition hover:brightness-110">
            {text.signinCta}
            <ArrowRight size={18} />
          </a>
        </div>
...
function SigninCard({
  ...
  return (
    <form onSubmit={submitLogin}>
      <h1 className="text-center text-4xl font-black text-ink">{copy.title}</h1>
      <GoogleButton locale={locale} label={text.google} />
      <Divider label={text.signinDivider} />
      <label className="block">
        <span className="mb-3 flex items-center gap-2 text-lg font-black"><Mail size={18} />{text.email}</span>
        <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="focus-ring w-full rounded-xl border border-ink/15 px-5 py-4 text-xl" />
      </label>
      <label className="mt-5 block">
        <span className="mb-3 block text-lg font-black">{text.password}</span>
        <div className="flex rounded-xl border border-ink/15">
          <input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} className="focus-ring min-w-0 flex-1 px-5 py-4 text-xl outline-none" />
          <button type="button" onClick={() => setShowPassword(!showPassword)} className="focus-ring px-4" aria-label={text.togglePassword}>
            {showPassword ? <EyeOff size={22} /> : <Eye size={22} />}
          </button>
        </div>
      ...
      {error ? <p className="mt-4 animate-fade-in rounded-xl border border-coral/30 bg-coral/10 px-3 py-2 text-sm font-bold text-coral">{error}</p> : null}
      <button disabled={busy} className="focus-ring mt-7 flex w-full items-center justify-center gap-2 rounded-2xl bg-tide px-5 py-4 text-lg font-black text-white shadow-soft transition hover:brightness-110 disabled:opacity-45">
        {busy ? text.loginBusy : copy.cta}
        <ArrowRight size={18} />
      </button>
      <a className="mt-6 block text-center text-base font-black text-tide" href={copy.switchHref}>{copy.switchText}</a>
    </form>
  );
}
